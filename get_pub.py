...
def get_phone(url):
	...
	try:
		phone = driver.find_element_by_xpath('//span[@class="detail  is-hidden-mobile"]')
		phone = phone.text
	except:
		phone = 'ERROR'
...
def get_address(url):
	...
	driver = url
	try: 
		street = driver.find_element_by_xpath('//span[@class="street-address"]')
		street = street.text
...
def get_info(driver):
	time.sleep(4)
	restaurants = ui.WebDriverWait(driver, 15).until(lambda driver: driver.find_elements_by_xpath('//a[@class="property_title"]'))
	...
	#Next.click()
	previous = driver.find_element_by_xpath('//a[@class="nav previous rndBtn ui_button primary taLnk"]') 
	previous.click()	
	print('\n\n****NEXT**** ')
	save(name,address,email,phone,website)
...
def next(url,contador,decrementador):
	driver = url
	try:
		Next = ui.WebDriverWait(driver, 15).until(lambda driver: driver.find_element_by_xpath('//a[@class="nav next rndBtn ui_button primary taLnk"]'))
		time.sleep(1)
		Next.click()
		decrementador = decrementador + 1
	except:
		print('ta na except, meu decrementador é:' + str(decrementador))
		time.sleep(4)
